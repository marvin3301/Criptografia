...
static void ConcatWithIv(uint8_t buf,uint8_t* iv)
{
...
void AES_CTR_encrypt(uint8_t* output, uint8_t* input, uint32_t length, const uint8_t* key, const uint8_t* iv)
{
    uintptr_t i;
    uint8_t extra = length % BLOCKLEN;
    ...
    for (i = 0; i < length; i += BLOCKLEN)
    {
        ConcatWithIv(count++,Iv);
        memcpy(output, input, BLOCKLEN);
        state = (state_t*)output;
        Cipher();
        int z;
    	for (z = 0; z < BLOCKLEN; z++)
            output[z] ^= input[z];
...
void AES_CTR_decrypt(uint8_t* output, uint8_t* input, uint32_t length, const uint8_t* key, const uint8_t* iv)
{
    AES_CTR_encrypt(input,output,length,key,iv);
}
